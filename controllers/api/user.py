...
# Perform Multiple action like GET, POST: delete,enable,disable,markasadmin,unmarkasadmin,markasglobaladmin,unmarkasglobaladmin
class List:
    ...
    # USAGE post string : mail=user1@domain.tld&mail=user2@domain.tld&action=delete&keep_mailbox_days=1
    @decorators.require_global_admin_api_key
...
class ListDisabled:
    ...
    def POST(self, domain):
        raise web.seeother('/api?msg=NO_POST_METHOD_FOR_THIS_ENDPOINT')
...
class Profile:
    ...
    def POST(self, mail):
        ...
        if result[0]:
            raise web.seeother('/api?msg=UPDATED')
        else:
            raise web.seeother('/api?msg={}'.format(web.urlquote(result[1])))

# Create Profile/User: POST , GET: 
class Create:
    @decorators.require_global_admin_api_key
    def GET(self, domain):
        domain = str(domain).lower()
